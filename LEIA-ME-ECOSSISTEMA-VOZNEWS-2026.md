# ECOSSISTEMA VOZ NEWS — atualização 07/08/2026

Esta versão preserva a estrutura original do site e acrescenta uma camada estratégica ao Portal-Mãe.

## Integrado nesta versão
- 40 portais/verticais clicáveis em `/portais/`
- VOZ Internacional
- indicadores de audiência e acervo
- autoridade de Paulo Fayad e Dra. Deijanete Fayad
- Academia VOZ NEWS
- expansão Voz dos Estados / Capitais / Cidades / Municípios
- novas opções comerciais no formulário
- estúdio e identidade visual preservados sem integrações sintéticas

## Arquivos de segurança
- `index-backup-antes-ecossistema-2026-08-07.html`: cópia do index recebido no ZIP
- `index.html`: nova versão integrada

## Antes de publicar
1. Fazer deploy de Preview no Vercel.
2. Testar menu, estúdio, ticker, formulário/WhatsApp e páginas de clientes.
3. Abrir alguns links em `/portais/` no desktop e celular.
4. Só então promover para Production.

## Métricas
A homepage mostra 197 mil+ seguidores no Instagram, 200 milhões+ de interações/mês, 500+ entrevistas e 10 mil+ horas de transmissão conforme dados institucionais informados pela TV Voz de Brasília. Manter esses números atualizados e documentados no mídia kit.


## Versão visual V4
- Corrigido enquadramento de imagens, com tratamento especial para Dra. Deijanete e Paulo Fayad.
- Removidas repetições de entrevistas na vitrine geral.
- Adicionada imagem de destaque aos 40 cards do Ecossistema e às 40 páginas de portais.
- Mantidas as fotografias reais existentes dos apresentadores; nenhuma imagem pessoal foi gerada por IA nesta revisão.
