...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    const sourceFile=presenter==='deijanete'?'studio-deijanete-source.jpg':'studio-paulo-source.jpg';
    const voice=await resolveVoice(presenter);
    const payload={
      source_url:`${origin}/${sourceFile}`,
      script:{
        type:'text',
        input:text,
    ...
    res.status(200).json({id:data.id,status:data.status,result_url:data.result_url||null,using_cloned_voice:voice.cloned,voice_id:voice.id});
  }catch(error){
