(() => {
  ...
  function renderFeatured(){
    if(!featured) return;
    featured.innerHTML = directory.map(item => `
      <article class="card">
        ...
        <div class="card-body">
          ...
          <a class="cta" href="${esc(item.url)}" ${/^https?:/.test(item.url) ? 'target="_blank" rel="noopener"' : ''}>${item.name === 'Pastelaria Viçosa' ? 'LER MATÉRIA' : 'CONHECER'}</a>
        </div>
      </article>`).join('');
  }
  ...
  function localCard(item){
    return `<div class="result"><strong>${esc(item.name)}</strong><small>ACERVO VOZ NEWS • ${esc(item.cuisine)}</small><p>${esc(item.address)}${item.phone ? `<br>${esc(item.phone)}` : ''}</p><a href="${esc(item.url)}" ${/^https?:/.test(item.url) ? 'target="_blank" rel="noopener"' : ''}>VER NO GUIA →</a></div>`;
  }

  function externalCard(item){
    const cuisine = item.cuisine || 'Restaurante';
    return `<div class="result"><strong>${esc(item.name || 'Restaurante')}</strong><small>DADOS PÚBLICOS • ${esc(cuisine)}</small><p>${esc(item.address || 'Endereço não informado')}</p>${item.mapUrl ? `<a href="${esc(item.mapUrl)}" target="_blank" rel="noopener">ABRIR NO MAPA →</a>` : ''}</div>`;
  }
  ...
  renderFeatured();
  if(results) results.innerHTML = directory.slice(0,3).map(localCard).join('') + '<div class="search-status">Digite um nome, cozinha ou destino para pesquisar.</div>';
})();
